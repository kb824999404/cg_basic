...
if __name__=='__main__':
    ...
    img = BresenhamCircle_square_brush(img,400,400,200,10,255)
    endtime = datetime.datetime.now()
    print("Time:{}ms".format((endtime - starttime).microseconds))
    showImg(img,"Circle Square Brush (width=10)","outputs/Circle_Square_Brush1.jpg")

    starttime = datetime.datetime.now()
    img = np.zeros([X,Y],dtype=np.uint8)
    img = BresenhamCircle_square_brush(img,400,400,200,50,255)
    endtime = datetime.datetime.now()
    print("Time:{}ms".format((endtime - starttime).microseconds))
    showImg(img,"Circle Square Brush (width=50)","outputs/Circle_Square_Brush2.jpg")